...
public class RTS_EventManager extends RTS_StatefulClasses {
    ...
    public String addEvent(RTS_Event newEvent) {
        String identifier = UUID.randomUUID().toString();
        this.events.put(identifier, newEvent);
        return(identifier);
    }

    public String addListener(RTS_Listener newListener) {
        String identifier = UUID.randomUUID().toString();
...
//        for (List<String> keyValuePair : this.listenersToBeDeleted) {
//            if (this.listeners.get(keyValuePair.get(0)) != null && this.listeners.get(keyValuePair.get(0)).isEmpty())
//                this.listeners.remove(keyValuePair.get(0));
//        }
        for (List<String> keyValuePair : this.listenersToBeDeleted)
            keyValuePair.clear();
