...
public class RTS_SelectionListener extends RTS_StatefulClasses {
    ...
    public void controlGroupHandler(Integer group, boolean add) {
        if (add)
            this.controlGroups.put(group, (List<ShipAPI>)this.getState("currentSelection"));
        else if (this.controlGroups.containsKey(group)) {
            this.showShipDetails(this.controlGroups.get(group));
            this.setState("currentSelection", this.controlGroups.get(group));
        }
    }
    ...
    private List <ShipAPI> createSelection (Vector2f coOrd) {
        ...
        if (Math.abs(xStart - xEnd) < 10 && Math.abs(yStart - yEnd) < 10)
    ...
    private void showShipDetails (List<ShipAPI> concludedSelection) {
        CombatEngineAPI engine = (CombatEngineAPI)this.getState("engine");
    ...
    private void drawSelectionLines(Vector2f screenSpace) {
        ...
        GL11.glVertex2d(
                (double)sBeginEvent.getX(),
                (double)sBeginEvent.getY()
        );
        GL11.glVertex2d(
                (double)sBeginEvent.getX(),
                (double)screenSpace.getY()
        );
        GL11.glVertex2d(
                (double)screenSpace.getX(),
                (double)screenSpace.getY()
        );
        GL11.glVertex2d(
                (double)screenSpace.getX(),
