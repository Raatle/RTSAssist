...
public class RTS_ShipLocAPI extends RTS_StatefulClasses {
    ...
    private void getShipList() {
        ...
        if (!addedShips.isEmpty()) {
            ((RTS_EventManager)this.getState("eventManager")).addEvent(this.newShipsEvent, true);
        }
        if (!removedShips.isEmpty())
            ((RTS_EventManager)this.getState("eventManager")).addEvent(this.lostShipsEvent, true);
        // Finish
