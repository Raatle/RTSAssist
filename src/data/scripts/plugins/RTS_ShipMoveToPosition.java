...
public class RTS_ShipMoveToPosition extends RTS_StatefulClasses {
    ...
    public void cleanup (HashMap<String, Object> assignment) {
        if (assignment.get("blockedSystem") != null)
            ((RTS_BlockSystemPlugin)assignment.get("blockedSystem")).cleanUp(
                    new RTS_Context(this.returnState(true), this.returnState(), assignment)
            );
        ((HashMap<String, Object>)this.getState("thrust")).clear();
        ((HashMap<String, Object>)this.getState("vData")).clear();
        ((HashMap<String, Object>)this.getState("shipHullStrafeMod")).clear();
        ((RTS_AIInjector)this.getState("AIInjector", true)).removeInjection(
                (ShipAPI)this.getState("ship"),
                (String)this.getState("AIHookHold"),
                RTS_AIInjector.eventType.PREADVANCE
        );
        ((RTS_Pathing)this.getState("pathing")).cleanUp();
        this.cleanState();
    }
...
//            System.out.println(((ShipAPI)((ShipAPI)assignment.get("ship")).getShipAI().getAIFlags().getCustom(ShipwideAIFlags.AIFlags.MANEUVER_TARGET)).getName());
        ...
        if (this.getState("ship") != null) {
            if (assignment.get("targetEnemy") != null
                    ...
                    || this.getState("closestEnemy") != null
            ) {
                ShipAPI enemy = assignment.get("targetEnemy") != null
                        ...
                        : (ShipAPI)this.getState("closestEnemy");
            ...
            this.setState("closestEnemy", null);
            if (update) this.update(
                    ...
                    ((HashMap<String, RTS_MoveTogetherManager>)this.getState("togaStore", true))
                            .get(((ShipAPI)this.getState("ship")).getId())
            );
            if (this.getState("AIHookHold") == null) {
                String hookHold = ((RTS_AIInjector)this.getState("AIInjector", true)).registerInjection(
                        (ShipAPI)this.getState("ship"),
                        RTS_AIInjector.eventType.PREADVANCE,
                        new RTS_AIInjectedEvent() {
                            @Override
                            public boolean shouldExecute() {
                                return (getState("thrust") != null);
                            }

                            @Override
                            public void run() {
                                applyThrust(
                                        (ShipAPI)getState("ship"),
                                        (HashMap<String, Integer>)getState("thrust")
                                );
                                applyRotation(
                                        (ShipAPI)getState("ship"),
                                        (Vector2f)assignment.get("modDest"),
                                        (int)getState("rotation"),
                                        (assignment.get("targetEnemy") != null
                                                || assignment.get("priorityEnemy") != null
                                                || getState("closestEnemy") != null)
                                );
                            }
                ...
                this.setState("AIHookHold", hookHold);
            }
            this.blockSystem(
    ...
    public void update (
        ...
        hold.put("distanceToPoint", RTS_AssortedFunctions.fastGetDistance(ship.getLocation(), coOrd));
        ...
        hold.put(
                ...
                this.adjustMaxSpeed(
                        ...
                        (float)((Integer)hold.get("distanceToPoint")),
                        ship,
        ...
        else if (this.getState("closestEnemy") != null)
            target = ((ShipAPI)this.getState("closestEnemy")).getLocation();
        else
            target = coOrd;
        this.getRotation(
                ship.getFacing(),
    ...
    private Vector2f findClosestEnemy(ShipAPI ship, Vector2f coOrd, HashMap<String, Object> assignment) {
        ...
        List<ShipAPI> targets = ((RTS_ShipLocAPI)this.getState("ShipLocAPI", true)).findEnemyShipNeighbours(
                ship,
        ...
        if (closest == null)
            return (coOrd);
        this.setState("closestEnemy", closest);
    ...
    private void drawBounds (ShipAPI ship) {
        RTS_Draw draw = ((RTS_Draw)this.getState("draw", true));
    ...
    private void getThrust (Vector2f relativeVelocity, Vector2f relativeTargetVector) {
        HashMap<String, Integer> thrust = (HashMap<String, Integer>)this.getState("thrust");
        thrust.clear();
        thrust.put("x", 0);
        thrust.put("y", 0);
    ...
    private void getRotation (float ship, float target, float angularVelocity, HashMap<String, Object> assignment) {
        ...
        ShipAPI enemy;
        if (this.getState("closestEnemy") != null)
            enemy = (ShipAPI)this.getState("closestEnemy");
        else
