...
//            System.out.println(((ShipAPI)((ShipAPI)assignment.get("ship")).getShipAI().getAIFlags().getCustom(ShipwideAIFlags.AIFlags.MANEUVER_TARGET)).getName());
    ...
    public void update (
        ...
        if ((RTS_TaskManager.assType)assignment.get("assType") == RTS_TaskManager.assType.attackMove) {
            coOrd = this.findClosestEnemyAttackPosition(ship, coOrd, assignment);
        }
    ...
    private Vector2f findClosestEnemyAttackPosition(ShipAPI ship, Vector2f coOrdRaw, HashMap<String, Object> assignment) {
        ...
        ShipAPI targetedShip = (ShipAPI)assignment.get("targetEnemy");
        Vector2f coOrd;
        if (targetedShip != null)
            coOrd = targetedShip.getLocation();
        else
            coOrd = coOrdRaw;
        ...
        assignment.put("closestEnemy", closest);
        if (targetedShip != null
                && targetedShip.getId().equals(closest.getId())) {
            return (this.findAttackCoOrd(ship, closest, true, assignment));
        }
        ...
        attackRange = (attackRange + (targetCollsionD2C -30f));
        float finalAttackRange = attackRange;
        ShipAPI finalClosest = closest;
        RTS_AssortedFunctions.LRSTest testFunc = new RTS_AssortedFunctions.LRSTest() {
            Vector2f testHold;
            ...
            public int test(float test) {
                testHold = VectorUtils.resize(VectorUtils.getDirectionalVector(coOrd, ship.getLocation()), test);
                testCoOrd.set(
                        coOrd.getX() + testHold.getX(),
                        coOrd.getY() + testHold.getY()
                );
                dHold = (float)(Math.pow(finalAttackRange, 2) - RTS_AssortedFunctions.getDistanceSquared(
        ...
        float startingVal = Math.abs(closest.getLocation().getX() - coOrd.getX())
                           +Math.abs(closest.getLocation().getY() - coOrd.getY())
                           +finalAttackRange;
    ...
    private Vector2f getWeaponRanges(ShipAPI ship, HashMap<String, Object> weaponPosMods) {
        ...
        while (flag < 3) {
                ...
                if (
                        ...
                        || (flag < 2 &&
                                (
                                        ...
                                        || set.getOriginalSpec().getPrimaryRoleStr().equals("Anti Fighter")
            ...
            flag = hold.getY() != 1000000f
                    ? 3f
                    : flag + 1f;
        }
        if (hold.getY() == 1000000f) {
            hold.set(
                    5000f,
                    5000f
            );
    ...
    private float getBroadSideModifier (ShipAPI ship, float target, boolean hasEnemy) {
        if (!hasEnemy)
            return (target);
        HashMap<String, Object> modifiers = (HashMap<String, Object>)this.getState(
                RTSAssist.stNames.broadsideData,
                true
        );
        if (!modifiers.containsKey(ship.getHullSpec().getBaseHullId()) && modifiers.get(ship.getId()) == null)
            return (target);
        Vector2f broadsideMod;
        if (modifiers.get(ship.getId()) != null)
            broadsideMod = (Vector2f)modifiers.get(ship.getId());
        else
            broadsideMod = (Vector2f)modifiers.get(ship.getHullSpec().getBaseHullId());
        if (broadsideMod.getY() <= 360f) {
            float rotation  = MathUtils.getShortestRotation(ship.getFacing(), target);
            if (rotation >= 0)
                return (target + broadsideMod.getX());
            else
                return (target + broadsideMod.getY());
        }
        else
            return (target + broadsideMod.getX());
    }
