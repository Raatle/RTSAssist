...
public class RTS_ShipTestSuite extends RTS_StatefulClasses {
    ...
    String testPack = "assortment_of_things";
    ...
    public RTS_ShipTestSuite(Object state) {
        ...
        this.inCampaign = Global.getSector().getMemory().get("$RTSASSIST_saveData") != null;
    ...
    public void spawnTestShip(Vector2f worldSpace) {
        String testShip;
        try {
            testShip = (String)this.commonsControl.get(this.comTest);
    ...
    public void __retreiveCommonsShipList (boolean update) throws JSONException, IOException {
        ...
        if (this.commonsControl.get(this.comArray) == null)
            this.commonsControl.set(this.comArray, new JSONArray());
        JSONArray extraction = (JSONArray)this.commonsControl.get(this.comArray);
    ...
    private void replaceDefaultShipWithTestShip () {
        if (this.inCampaign)
            return;
        try {
            String testShip = (String)this.commonsControl.get(this.comTest);
            if (testShip == null || testShip.isEmpty())
                return;
        } catch (JSONException e) {
            throw new RuntimeException(e);
        ...
        ((RTS_EventManager)this.getState(RTSAssist.stNames.eventManager, true)).addEvent(new RTS_Event() {
            ...
            public void run() {
                ...
                spawnTestShip(replace.getLocation());
            }
        });
