...
public class RTS_ShipTestSuite extends RTS_StatefulClasses {
    ...
    public RTS_ShipTestSuite(Object state) {
        super(state, true);
        String modID = (String)this.getDeepState(Arrays.asList(
                        RTSAssist.stNames.config,
                        RTSAssist.devNames.modID),
                true
        );
        this.testPack = modID.equals("") && this.testPack.equals("")
                ? ""
                : modID;
        this.allShips = (boolean)this.getDeepState(Arrays.asList(
                        RTSAssist.stNames.config,
                        RTSAssist.devNames.findAllShips
                ),
                true
        );
        ...
        for (String x:  Global.getSettings().getAllVariantIds()) {
            ...
            if (
                    (SHSAPointer.getSourceMod() == null && !this.testPack.isEmpty())
                            ...
                            || (blockedSystems.contains(SHSAPointer.getShipSystemId())
                            && !this.allShips
                            &&  !this.testPack.isEmpty())
                            || (SHSAPointer.getShipSystemId() == null && !this.allShips)
            )
                continue;
            if (
